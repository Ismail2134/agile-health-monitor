<script>
  ...
  function confirmDelete(team) {
    if (window.confirm("Team '" + team.team + "' verwijderen? Dit verwijdert het team bij ALLE gebruikers.")) {
...
<svelte:head>
  <title>Teams beheren - Team Dashboard</title>
</svelte:head>

<h1 class="mb-6 text-2xl font-bold text-gray-900 dark:text-white">Mijn teams</h1>
...
<h2 class="mb-3 text-lg font-semibold text-gray-900 dark:text-white">Teams ({data?.teams?.length ?? 0})</h2>
