<script>
  ...
  $effect(() => {
    if (!isAdmin && userTeams.length > 0 && !sessionTeam) {
      sessionTeam = userTeams[0];
    }
    if (!isAdmin && userTeams.length > 0 && !sprintTeam) {
      sprintTeam = userTeams[0];
    }
  });
...
<svelte:head><title>Vote sessies - Team Dashboard</title></svelte:head>

<h1 class="mb-2 text-2xl font-bold text-gray-900 dark:text-white">Vote sessies</h1>
<div class="mb-6 flex flex-wrap gap-2">
  <a href="/team-dashboard" class="btn-secondary btn-xs no-underline">&larr; Terug naar dashboard</a>
  {#if !isAdmin}
...
<details class="mb-6 rounded-xl border border-gray-200 dark:border-gray-700">
  <summary class="cursor-pointer rounded-xl bg-gray-50 px-4 py-3 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-100 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700 list-none [&::-webkit-details-marker]:hidden">
    + Nieuwe sprint toevoegen
  </summary>
  <form method="POST" action="?/addSprint" class="space-y-4 border-t border-gray-200 p-4 dark:border-gray-700">
    <div class="grid gap-4 sm:grid-cols-2">
      <label class="block">
        <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Team</span>
        {#if isAdmin}
          <select name="team" required bind:value={sprintTeam} class="input-field mt-1">
            <option value="">-- Selecteer team --</option>
            {#each data?.teams ?? [] as t}
              <option value={t.team}>{t.team}</option>
            {/each}
      ...
      <label class="block">
        ...
        <input type="text" name="name" bind:value={sprintName} required maxlength="50" placeholder="S26-01" class="input-field mt-1" />
      </label>
    </div>
    <button type="submit" class="btn-primary">Sprint aanmaken</button>
  </form>
</details>
...
{#if (data?.sessions?.length ?? 0) === 0}
  <p class="text-sm text-gray-500 dark:text-gray-400">Nog geen sessies aangemaakt voor jouw teams.</p>
{:else}
